...
copy_panel_revision_data()
{
    ...
    while [ "$wait_cnt" -lt 8 ]; do
        if [ -e ${PATH_DISPLAY_DRM}/panelName -o -e ${PATH_DISPLAY}/panelName ]; then
            ...
            if [[ -z "$HNAME" && -e ${PATH_DISPLAY_DEVICETREE}/mmi,panel_name ]] ; then
                HNAME=`cat ${PATH_DISPLAY_DEVICETREE}/mmi,panel_name`
            fi
            if [[ -z "$VEND" && -e ${PATH_DISPLAY_DEVICETREE}/mmi,panel_supplier ]] ; then
                VEND=`cat ${PATH_DISPLAY_DEVICETREE}/mmi,panel_supplier`
            fi
            if [[ -z "$HREV" && -e ${PATH_DISPLAY_DEVICETREE}/mmi,panel_ver ]] ; then
                HREV=`cat ${PATH_DISPLAY_DEVICETREE}/mmi,panel_ver`
            fi
